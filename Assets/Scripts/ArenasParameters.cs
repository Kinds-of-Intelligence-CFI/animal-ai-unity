...
namespace ArenasParameters
{
    ...
    public class ArenasConfigurations
    {
        ...
        public void UpdateWithYAML(YAMLDefs.ArenaConfig yamlArenaConfig)
        {
            ...
            {
                if (existingIds.Contains(arenaConfiguration.Key))
                {
                    Debug.LogWarning($"Duplicate arenaID: {arenaConfiguration.Key} found. Assigning a new unique ID: {nextAvailableId} to the latter arena.");
                    Add(nextAvailableId, arenaConfiguration.Value);
                    existingIds.Add(nextAvailableId);

                    // Update next available ID for potential future additions.
                    nextAvailableId++;
                    while (existingIds.Contains(nextAvailableId))
                    {
                        nextAvailableId++;
                ...
                else if (arenaConfiguration.Key < 0)
                {
                    Debug.LogWarning($"Arena with ID {arenaConfiguration.Key} has been changed to {nextAvailableId}.");
                    Add(nextAvailableId, arenaConfiguration.Value);
                    existingIds.Add(nextAvailableId);
